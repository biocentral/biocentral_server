        ...
        if embd_record.get_set() == "train":
            ...
            if config_dict["discrete"]:
                if raw_target in target_classes:
                    ...
                    ValueError(
                        f"get_datasets: illegal label {raw_target} in labels: {target_classes.keys()}"
                    )
            else:
...
def data_prep(
    ...
    embd_records = [
        id2record[seq_record.seq_id].copy_with_embedding(seq_record.embedding)
    ...
    if len(train_data["ids"]) * len(inference_data) == 0:
        raise ValueError(
            "data_prep: training set / inference set is empty. Have you set feature_name?"
        )
