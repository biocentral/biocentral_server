...
class DatabaseLoadEmbeddingStep(PipelineStep):
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        reduced = (
            Protocol.from_string(context.config["protocol"])
            in Protocol.using_per_sequence_embeddings()
        )

        seq_records = read_FASTA(input_file)
        seq_dict = {seq_record.get_hash(): seq_record.seq for seq_record in seq_records}
        embeddings = self.embeddings_db.get_embeddings(
            sequences=seq_dict, embedder_name=self.embedder_name, reduced=reduced
        )
        context.id2emb = {
            embd_record.get_hash(): embd_record.seq for embd_record in embeddings
        }
        return context
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        embd_record_tuples = list(
            embedding_service.generate_embeddings(
                input_data=seq_records, reduce=reduced
            )
        )
        ...
        context.id2emb = {
            ...
            for seq_record, embd in embd_record_tuples
        }
        return context
