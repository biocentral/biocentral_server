"""Download ONNX prediction models for standalone testing.

This script downloads ONNX models from TUM Nextcloud without any server dependencies.
Can be used standalone or imported as a module for pytest fixtures.

Usage:
    python -m tests.fixtures.download_onnx_models [--output-dir /path/to/models]
    
The downloaded models are extracted to the specified directory (default: ./onnx_models).
Set ONNX_MODELS_PATH environment variable to point tests to the downloaded models.
"""
...
def download_and_extract(
    ...
    """Download and extract ONNX models from the provided URLs.
    
    Args:
        urls: List of URLs to try (fallback order)
        output_dir: Directory to extract models to
        force: If True, re-download even if directory exists
        
    Returns:
        Path to the extracted models directory
...
def get_or_download_models(
    ...
    """Get the path to ONNX models, downloading if necessary.
    
    This function checks ONNX_MODELS_PATH first, then falls back to downloading.
    
    Args:
        output_dir: Directory to download to (default: ./onnx_models)
