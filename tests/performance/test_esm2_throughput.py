...
class TestESM2SingleSequenceLatency:
    ...
    def test_very_long_sequence_latency(
        self, esm2_embedder, very_long_sequence, benchmark
    ):
...
class TestESM2BatchThroughput:
    ...
    def test_canonical_dataset_throughput(
        self, esm2_embedder, canonical_sequences, benchmark
    ):
...
class TestESM2Comparison:
    ...
    def test_esm2_vs_fixed_embedder(
        self, esm2_embedder, perf_embedder, medium_sequence
    ):
        ...
        print(f"{'=' * 50}")
        print(
            f"FixedEmbedder: {fixed_mean:.3f} ms (±{np.std(fixed_times) * 1000:.3f} ms)"
        )
        print(
            f"ESM2-t6-8M:    {esm2_mean:.3f} ms (±{np.std(esm2_times) * 1000:.3f} ms)"
        )
...
class TestESM2ScalingWithLength:
    ...
    def test_scaling_report(self, esm2_embedder, variable_length_sequences):
        ...
        print(f"{'=' * 60}")
        print(
            f"{'Length':<10} {'Mean (ms)':<15} {'Std (ms)':<15} {'Throughput (seq/s)':<20}"
        )
        print(f"{'-' * 60}")
        ...
        for sequence in variable_length_sequences:
            ...
            print(
                f"{len(sequence):<10} {mean_ms:<15.3f} {std_ms:<15.3f} {throughput:<20.1f}"
            )
        ...
        print(
            f"{len(variable_length_sequences):<10} {mean_ms:<15.3f} {std_ms:<15.3f} {throughput:<20.1f}"
        )
